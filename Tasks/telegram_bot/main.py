...
async def process_start_command(message: types.Message):
    ...
    if data is None:
        ...
        await message.reply(f"Привет, а я тебя помню!")
...
async def process_start_command(message: types.Message):
    ...
    connect.commit()
    await message.reply(f"Тебя больше нет в базе:(\n"
                        f"Для того чтобы вернутся пиши /start.")


@dp.message_handler(commands=['help'])
async def process_help_command(message: types.Message):
    await message.reply(f"Привет, я расскажу тебе о твоих успехах в алгоритмах и структурах данных!")


@dp.message_handler(commands=['mark'])
async def get_mark(message: types.Message):
    try:
        all_table_ranges = "A1:R132"
        first_req = requests.get(
            f"https://sheets.googleapis.com/v4/spreadsheets/1ha9zc_2ZuPEqfI5f3V1LyTt9R9hlGw7RsoHmHDGJVvU/"
            f"values/{all_table_ranges}?key={os.getenv('api_key')}"
        )

        limits_table_ranges = "N5:U7"
        third_req = requests.get(
            f"https://sheets.googleapis.com/v4/spreadsheets/1ha9zc_2ZuPEqfI5f3V1LyTt9R9hlGw7RsoHmHDGJVvU/"
            f"values/{limits_table_ranges}?key={os.getenv('api_key')}"
        )
        ...
        mark = int(all_table_data['values'][index_in_table][-1])

        contest_mark = all_table_data['values'][index_in_table][4]
        ...
        if mark < 9:
            ...
            await message.reply(f"На данный момент твоя итоговая оценка {mark}.\n"
                                ...
                                f"Для оценки {mark + 1} необходимо набрать {difference} {ending}.")
        else:
            await message.reply(f"На данный момент твоя итоговая оценка {mark}.\n"
                                f"Балл за контесты {all_table_data['values'][index_in_table][4]}.\n"
                                f"Оценка за контесты {all_table_data['values'][index_in_table][5]}.\n"
...
async def get_mark(message: types.Message):
    try:
        ...
        first_req = requests.get(
            f"https://sheets.googleapis.com/v4/spreadsheets/1ha9zc_2ZuPEqfI5f3V1LyTt9R9hlGw7RsoHmHDGJVvU/"
            f"values/{sheet_name}!{all_table_ranges}?key={os.getenv('api_key')}"
        )
...
async def get_full_name(message: types.Message):
    #try:
        full_name_table_ranges = "BE4:BU132"
        second_req = requests.get(
            f"https://sheets.googleapis.com/v4/spreadsheets/1ha9zc_2ZuPEqfI5f3V1LyTt9R9hlGw7RsoHmHDGJVvU/"
            f"values/Контесты!{full_name_table_ranges}?key={os.getenv('api_key')}"
        )
        full_names_data = second_req.json()
        pprint(full_names_data['values'][9])
        # index_in_table = 1000
